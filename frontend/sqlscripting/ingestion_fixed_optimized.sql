...
BEGIN
    ...
    IF EXISTS(SELECT 1 FROM filtered WHERE Codes IS NOT NULL AND TRIM(Codes) != '') THEN
        ...
        -- Insert valid codes that exist in attributes table
        ...
        -- Flag measurements with invalid codes for validation
        -- Invalid codes are those in tempcodes but not in attributes table
