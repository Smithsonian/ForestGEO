...
SELECT c.CensusID,
       ...
       COALESCE(MAX(d.ExactDate), c.EndDate)   AS EndDate, LEFT (c.Description, 65535), c.PlotCensusNumber
FROM
    stable_sinharaja.census c
    LEFT JOIN
    stable_sinharaja.dbh d
ON c.CensusID = d.CensusID
GROUP BY
    c.CensusID
ON DUPLICATE KEY
UPDATE
...
SELECT CensusID,
       ...
       LastName,
       RoleID
FROM tmp_personnel ON DUPLICATE KEY
UPDATE
    RoleID =
VALUES (RoleID);
