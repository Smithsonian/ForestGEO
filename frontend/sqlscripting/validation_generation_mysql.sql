create
    definer = azureroot@`%` procedure forestgeo_bci.CheckIfQuadratByNameExistsInPlot(IN quadratName text, IN plotID int)
BEGIN
...
create
    definer = azureroot@`%` procedure forestgeo_bci.CheckIfTreeTagExists(IN inputTreeTag varchar(10))
BEGIN
    SELECT EXISTS (SELECT 1
                   FROM forestgeo_bci.trees
                   WHERE TreeTag = inputTreeTag) AS TreeTagExists;
END;

create
    definer = azureroot@`%` procedure forestgeo_bci.CountNumDeadMissingStems()
BEGIN
    SELECT cm.CensusID,
           a.Status,
    ...
    FROM forestgeo_bci.coremeasurements cm
             JOIN
         forestgeo_bci.stems s ON cm.StemID = s.StemID
             JOIN
         forestgeo_bci.cmattributes cma ON cm.CoreMeasurementID = cma.CoreMeasurementID
             JOIN
...
create
    definer = azureroot@`%` procedure forestgeo_bci.CountNumRecordsByQuadrat()
BEGIN
    SELECT q.PlotID,
           cm.CensusID,
           q.QuadratName,
    ...
    FROM forestgeo_bci.quadrats q
             JOIN
         forestgeo_bci.stems s ON q.QuadratID = s.QuadratID
             JOIN
         forestgeo_bci.coremeasurements cm ON s.StemID = cm.StemID
             JOIN
         forestgeo_bci.trees t ON s.TreeID = t.TreeID
             LEFT JOIN
         forestgeo_bci.cmattributes cma ON cm.CoreMeasurementID = cma.CoreMeasurementID
             LEFT JOIN
         forestgeo_bci.attributes a ON cma.Code = a.Code
...
BEGIN
    ...
    DECLARE
        vStemID INT;
    DECLARE
        vDBH DECIMAL(10, 2); -- Ensure compatibility with the table definition
    DECLARE
        vHOM DECIMAL(10, 2); -- Ensure compatibility with the table definition
    DECLARE
        vExactDate DATE;
    DECLARE
        vComments VARCHAR(128);
    DECLARE
        vPlotID INT;
    DECLARE
        vQuadratID INT;
    DECLARE
        vTreeID INT;
    DECLARE
        vIsRemeasurement TINYINT(1);
    DECLARE
        vRowCount INT DEFAULT 0;
    DECLARE
        vMaxRows INT; -- Variable to store the maximum number of rows to insert
    DECLARE
        done INT DEFAULT FALSE;

    -- Declare a cursor for a simulated FULL OUTER JOIN result set from dbh and remeasurements
    DECLARE
        combinedCursor CURSOR FOR
...
-- Insert a row for DBH measurement or remeasurement
        ...
        VALUES (vCensusID, vPlotID, vQuadratID, vTreeID, vStemID, CAST(vDBH AS DECIMAL(10, 2)),
                CAST(vHOM AS DECIMAL(10, 2)),
                vExactDate, vComments, vIsRemeasurement, FALSE);
...
-- Update the IsCurrent field for the most recent measurement of each stem
    UPDATE forestgeo_bci.CoreMeasurements cm
        INNER JOIN (SELECT MAX(CoreMeasurementID) AS LatestMeasurementID, StemID
                    ...
                    GROUP BY StemID) AS latest
        ON cm.CoreMeasurementID = latest.LatestMeasurementID
    SET cm.IsCurrent = TRUE;
END;

create
    definer = azureroot@`%` procedure forestgeo_bci.ValidateDBHGrowthExceedsMax(IN p_CensusID int, IN p_PlotID int)
begin
    DECLARE
    ...
    DECLARE
        cur CURSOR FOR
            ...
            FROM forestgeo_bci.coremeasurements cm1
                     JOIN forestgeo_bci.coremeasurements cm2
                          ON cm1.StemID = cm2.StemID
                              AND cm1.TreeID = cm2.TreeID
                              AND YEAR(cm2.MeasurementDate) = YEAR(cm1.MeasurementDate) + 1
                     LEFT JOIN forestgeo_bci.cmattributes cma
                               ON cm1.CoreMeasurementID = cma.CoreMeasurementID
                     LEFT JOIN forestgeo_bci.attributes a
                               ON cma.Code = a.Code
            WHERE (a.Status NOT IN ('dead', 'stem dead', 'broken below', 'missing', 'omitted')
                OR
                   a.Status IS NULL)
              ...
              AND (p_CensusID IS NULL
                OR cm2.CensusID = p_CensusID)
              AND (p_PlotID IS NULL
                OR cm2.PlotID = p_PlotID);
    DECLARE
        CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;
    ...
    LOOP
        ...
        VALUES (vCoreMeasurementID, 14);
        SET
            insertCount = insertCount + 1;
        UPDATE forestgeo_bci.coremeasurements
...
create
    definer = azureroot@`%` procedure forestgeo_bci.ValidateDBHShrinkageExceedsMax(IN p_CensusID int, IN p_PlotID int)
BEGIN
    DECLARE
        vCoreMeasurementID INT;
    DECLARE
...
create
    definer = azureroot@`%` procedure forestgeo_bci.ValidateFindAllInvalidSpeciesCodes(IN p_CensusID int, IN p_PlotID int)
BEGIN
    DECLARE
        vCoreMeasurementID INT;
    DECLARE
    ...
    DECLARE
        cur CURSOR FOR
            ...
            FROM forestgeo_bci.stems s
                     JOIN forestgeo_bci.trees t ON s.TreeID = t.TreeID
                     LEFT JOIN forestgeo_bci.species sp ON t.SpeciesID = sp.SpeciesID
...
create
    definer = azureroot@`%` procedure forestgeo_bci.ValidateFindDuplicateStemTreeTagCombinationsCurrentCensus(IN p_CensusID int, IN p_PlotID int)
BEGIN
    DECLARE
        vCoreMeasurementID INT;
    DECLARE
    ...
    DECLARE
        cur CURSOR FOR
            ...
            FROM forestgeo_bci.coremeasurements cm
                     ...
                     JOIN (SELECT MAX(CensusID) AS LatestCensusID FROM forestgeo_bci.census) latest
...
create
    definer = azureroot@`%` procedure forestgeo_bci.ValidateFindDuplicateStemTreeTagCombinationsPerCensus(IN p_CensusID int, IN p_PlotID int)
BEGIN
    DECLARE
        vCoreMeasurementID INT;
    DECLARE
...
create
    definer = azureroot@`%` procedure forestgeo_bci.ValidateFindDuplicatedQuadratsByName(IN p_CensusID int, IN p_PlotID int)
BEGIN
    DECLARE
        vCoreMeasurementID INT;
    DECLARE
...
create
    definer = azureroot@`%` procedure forestgeo_bci.ValidateFindMeasurementsOutsideCensusDateBoundsGroupByQuadrat(IN p_CensusID int, IN p_PlotID int)
BEGIN
    DECLARE
        vCoreMeasurementID INT;
    DECLARE
    ...
    DECLARE
        cur CURSOR FOR
            ...
            FROM forestgeo_bci.coremeasurements cm
                     ...
                     JOIN forestgeo_bci.census c ON q.PlotID = c.PlotID
...
create
    definer = azureroot@`%` procedure forestgeo_bci.ValidateFindStemsInTreeWithDifferentSpecies(IN p_CensusID int, IN p_PlotID int)
BEGIN
    DECLARE
        vCoreMeasurementID INT;
    DECLARE
    ...
    DECLARE
        cur CURSOR FOR
            ...
            FROM forestgeo_bci.stems s
                     JOIN forestgeo_bci.trees t ON s.TreeID = t.TreeID
                     JOIN forestgeo_bci.species sp ON t.SpeciesID = sp.SpeciesID
                     JOIN forestgeo_bci.coremeasurements cm ON s.StemID = cm.StemID
            WHERE cm.IsValidated IS FALSE
              AND (p_CensusID IS NULL OR cm.CensusID = p_CensusID)
              AND (p_PlotID IS NULL OR cm.PlotID = p_PlotID)
...
create
    definer = azureroot@`%` procedure forestgeo_bci.ValidateFindStemsOutsidePlots(IN p_CensusID int, IN p_PlotID int)
BEGIN
    DECLARE
        vCoreMeasurementID INT;
    DECLARE
    ...
    DECLARE
        cur CURSOR FOR
            SELECT cm.CoreMeasurementID
            FROM forestgeo_bci.stems s
                     INNER JOIN forestgeo_bci.quadrats q ON s.QuadratID = q.QuadratID
                     INNER JOIN forestgeo_bci.plots p ON q.PlotID = p.PlotID
                     INNER JOIN forestgeo_bci.coremeasurements cm ON s.StemID = cm.StemID
...
create
    definer = azureroot@`%` procedure forestgeo_bci.ValidateFindTreeStemsInDifferentQuadrats(IN p_CensusID int, IN p_PlotID int)
BEGIN
    DECLARE
        vCoreMeasurementID INT;
    DECLARE
    ...
    DECLARE
        cur CURSOR FOR
            ...
            FROM forestgeo_bci.stems s1
                     JOIN forestgeo_bci.stems s2 ON s1.TreeID = s2.TreeID AND s1.StemID != s2.StemID
                     JOIN forestgeo_bci.coremeasurements cm1
                          ON s1.StemID = cm1.StemID
            WHERE s1.QuadratID != s2.QuadratID
              AND cm1.IsValidated IS FALSE
              AND (p_CensusID IS NULL
                OR cm1.CensusID = p_CensusID)
              AND (p_PlotID IS NULL
                OR cm1.PlotID = p_PlotID)
...
create
    definer = azureroot@`%` procedure forestgeo_bci.ValidateFindTreesWithMultiplePrimaryStems(IN p_CensusID int, IN p_PlotID int)
BEGIN
    DECLARE
        vCoreMeasurementID INT;
    DECLARE
...
create
    definer = azureroot@`%` procedure forestgeo_bci.ValidateHOMUpperAndLowerBounds(IN p_CensusID int,
                                                                                   IN p_PlotID int,
...
create
    definer = azureroot@`%` procedure forestgeo_bci.ValidateScreenMeasuredDiameterMinMax(IN p_CensusID int,
                                                                                         IN p_PlotID int,
...
create
    definer = azureroot@`%` procedure forestgeo_bci.ValidateScreenStemsWithMeasurementsButDeadAttributes(IN p_CensusID int, IN p_PlotID int)
BEGIN
    DECLARE
        vCoreMeasurementID INT;
    DECLARE
