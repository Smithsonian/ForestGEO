...
--   - Support retry bookkeeping, and job detail/event history
--
-- Keep this in sync with frontend/lib/background-jobs/catalog.ts, which bootstraps
-- these tables from the deployed app bundle.
--
-- IMPORTANT — UNRELEASED FEATURE:
--   This feature has never been deployed to production. No live data exists.
--   Breaking DDL changes (ENUM changes, dropped columns) are intentional.
--
--   If you have a pre-existing dev or test database that was bootstrapped from
--   an older version of this file, you MUST manually drop the tables before
--   running this script — the old ENUM values ('created', 'dead_lettered',
--   'blob_received') and the old LastMessageID column are incompatible:
--
--     DROP TABLE IF EXISTS
--       catalog.background_job_events,
--       catalog.background_job_files,
--       catalog.background_jobs;
-- =====================================================================================
...
(
    ...
    Status            ENUM ('queued','running','cancel_requested','waiting_retry','completed','failed','cancelled') DEFAULT 'queued'                       NOT NULL,
    Phase             ENUM ('queued','staging','ingestion','collapsing','validation','refreshing_views','completed','failed','cancelled') DEFAULT 'queued'  NOT NULL,
