...
const DATA_TYPE = 'plots';
const LOOKUP_COLUMN = 'PlotID';
...
describe('GET /api/formvalidation/[dataType]/[[...slugs]] authz', () => {
  ...
  function makeRequest() {
    return new Request('http://localhost/api/formvalidation') as any;
  }

  function makeContext(overrideSchema: string, value: string) {
    return { params: Promise.resolve({ dataType: DATA_TYPE, slugs: [overrideSchema, LOOKUP_COLUMN, value] }) } as any;
  }
  ...
  it('denies an out-of-scope schema with 403 and runs no SQL', async () => {
    ...
    const { GET } = await import('@/app/api/formvalidation/[dataType]/[[...slugs]]/route');
    const res = await GET(makeRequest(), makeContext(schema, String(plotID)));
  ...
  it('rejects a structurally invalid schema with 400 and runs no SQL', async () => {
    const { GET } = await import('@/app/api/formvalidation/[dataType]/[[...slugs]]/route');
    const res = await GET(makeRequest(), makeContext(INVALID_SCHEMA, String(plotID)));
  ...
  it('runs the lookup query for an authorized member request (200 for an existing value)', async () => {
    const { GET } = await import('@/app/api/formvalidation/[dataType]/[[...slugs]]/route');
    const res = await GET(makeRequest(), makeContext(schema, String(plotID)));
  ...
  it('returns 404 (not 403/400) for an authorized member when the value does not exist', async () => {
    const { GET } = await import('@/app/api/formvalidation/[dataType]/[[...slugs]]/route');
    const res = await GET(makeRequest(), makeContext(schema, '2147483647'));
