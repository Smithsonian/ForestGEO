...
describe('GET /api/verifysession authz', () => {
  ...
  it('denies an out-of-scope schema with 403 and runs no SQL', async () => {
    ...
    const { GET } = await import('@/app/api/verifysession/route');
    const req = new NextRequest(`http://localhost/api/verifysession?schema=${schema}&plotID=${plotID}&censusID=${censusID}`);
  ...
  it('rejects a structurally invalid schema with 400 and runs no SQL', async () => {
    const { GET } = await import('@/app/api/verifysession/route');
    const req = new NextRequest(`http://localhost/api/verifysession?schema=foo%3BDROP&plotID=${plotID}&censusID=${censusID}`);
    const res = await GET(req as any, EMPTY_ROUTE_CONTEXT);
  ...
  it('returns 200 with session verification counts for an authorized member request', async () => {
    const { GET } = await import('@/app/api/verifysession/route');
    const req = new NextRequest(`http://localhost/api/verifysession?schema=${schema}&plotID=${plotID}&censusID=${censusID}`);
    const res = await GET(req as any, EMPTY_ROUTE_CONTEXT);
    ...
    // The route ran its count queries against the real schema.
    const cm = (await import('@/lib/db/connectionmanager')).default.getInstance();
