...
// subpath) from being loaded by Node's native ESM resolver.
vi.mock('@/auth', () => ({
  auth: vi.fn(async () => ({
    user: {
...
// and count invocations so denial tests can prove NO SQL ran.
...
function buildRequest(schema: string, plotID: number, censusID: number): NextRequest {
  return new NextRequest('http://localhost/api/errors/explorer/facets', {
    method: 'POST',
...
describe('POST /api/errors/explorer/facets per-site authz', () => {
  ...
  it('denies a schema outside the user site scope with 403 and never runs any SQL', async () => {
    ...
    const { POST } = await import('@/app/api/errors/explorer/facets/route');
    // Body carries the in-scope real test schema; the authenticated user does NOT own it.
    const res = await POST(buildRequest(schema, plotID, censusID), { params: Promise.resolve({}) });

    expect(res.status).toBe(HTTP_FORBIDDEN);
    // The 403 must short-circuit before the handler touches the database.
  ...
  it('rejects a malformed schema in the body with 400 before running any SQL', async () => {
    sharedState.executeQueryCalls = 0;
    const { POST } = await import('@/app/api/errors/explorer/facets/route');
    const res = await POST(buildRequest(INVALID_SCHEMA, plotID, censusID), { params: Promise.resolve({}) });

    expect(res.status).toBe(HTTP_BAD_REQUEST);
    expect(sharedState.executeQueryCalls).toBe(0);
  });

  it('serves facets for an authorized member and proves the handler still reads the body after the guard clone', async () => {
    sharedState.executeQueryCalls = 0;
    const { POST } = await import('@/app/api/errors/explorer/facets/route');
    const res = await POST(buildRequest(schema, plotID, censusID), { params: Promise.resolve({}) });

    expect(res.status).toBe(HTTP_OK);
    // The handler read schema/plotID/censusID from the (post-clone) body and issued queries.
    expect(sharedState.executeQueryCalls).toBeGreaterThan(0);
    const body = await res.json();
    expect(body).toHaveProperty('messages');
    expect(body).toHaveProperty('fields');
    expect(body).toHaveProperty('sourceCounts');
  });
});
