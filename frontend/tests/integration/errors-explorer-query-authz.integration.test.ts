...
function buildRequest(schema: string, plotID: number, censusID: number): NextRequest {
  return new NextRequest('http://localhost/api/errors/explorer/query', {
    method: 'POST',
...
describe('POST /api/errors/explorer/query per-site authz', () => {
  ...
  it('denies a schema outside the user site scope with 403 and never runs any SQL', async () => {
    ...
    const { POST } = await import('@/app/api/errors/explorer/query/route');
  ...
  it('serves rows for an authorized member and proves the handler still reads the body after the guard clone', async () => {
    sharedState.executeQueryCalls = 0;
    const { POST } = await import('@/app/api/errors/explorer/query/route');
    const res = await POST(buildRequest(schema, plotID, censusID), { params: Promise.resolve({}) });
