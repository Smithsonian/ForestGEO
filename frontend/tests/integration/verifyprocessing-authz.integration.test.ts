...
describe('GET /api/verifyprocessing authz', () => {
  ...
  it('denies an out-of-scope schema with 403 and runs no SQL', async () => {
    ...
    const { GET } = await import('@/app/api/verifyprocessing/route');
    const req = new NextRequest(`http://localhost/api/verifyprocessing?schema=${schema}&plotID=${plotID}&censusID=${censusID}`);
  ...
  it('rejects a structurally invalid schema with 400 and runs no SQL', async () => {
    const { GET } = await import('@/app/api/verifyprocessing/route');
    const req = new NextRequest(`http://localhost/api/verifyprocessing?schema=foo%3BDROP&plotID=${plotID}&censusID=${censusID}`);
    const res = await GET(req as any, EMPTY_ROUTE_CONTEXT);
  ...
  it('returns 200 with processing counts for an authorized member request', async () => {
    const { GET } = await import('@/app/api/verifyprocessing/route');
    const req = new NextRequest(`http://localhost/api/verifyprocessing?schema=${schema}&plotID=${plotID}&censusID=${censusID}`);
    const res = await GET(req as any, EMPTY_ROUTE_CONTEXT);
    ...
    expect(body).toHaveProperty('remainingCount');
    expect(body).toHaveProperty('totalAccounted');
    expect(body.schema).toBe(schema);
    // The route ran at least the temp/processed/failed count queries against the real schema.
    const cm = (await import('@/lib/db/connectionmanager')).default.getInstance();
