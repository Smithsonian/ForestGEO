...
function buildRequest(schema: string, plotID: number): NextRequest {
  return new NextRequest('http://localhost/api/changes/explorer/facets', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ schema, plotID })
  });
}

describe('POST /api/changes/explorer/facets per-site authz', () => {
  ...
  it('denies a schema outside the user site scope with 403 and never runs any SQL', async () => {
    ...
    const { POST } = await import('@/app/api/changes/explorer/facets/route');
  ...
  it('serves facets for an authorized member and proves the handler still reads the body after the guard clone', async () => {
    ...
    expect(body).toHaveProperty('operationCounts');
  });
});
