-- =====================================================================================
-- Schema baseline: OLDEST PROVISIONABLE RELEASE (captured 2026-07-29 from branch `main`)
-- =====================================================================================
-- Verbatim snapshot of tablestructures.sql as shipped by the LIVE site, which deploys
-- from `main` — at capture time roughly three weeks behind the dev branch.
--
-- WHY IT EXISTS: provisioning bakes the DEPLOYED app's DDL into every new site, so a site
-- created through the live site starts at THIS shape, not the canonical one. Every object
-- the canonical DDL has and this one lacks must therefore be reachable from the migration
-- manifest, or that site can never be brought up to date — and, as happened on 2026-07-29
-- with forestgeo_niobrara, it fails the deploy-time contract gate and blocks the whole
-- pipeline for every schema.
--
-- schema-manifest-convergence.integration.test.ts loads this, applies the manifest, and
-- asserts nothing canonical is left behind. It is a FIXTURE, not a migration — nothing
-- executes it against a real deployment.
--
-- WHEN TO BUMP: once no live release this old can still provision a site (i.e. after `main`
-- catches up), re-capture from the new oldest deployed branch.
-- =====================================================================================
