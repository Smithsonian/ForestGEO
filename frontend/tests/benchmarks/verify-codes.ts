...
async function main() {
  ...
  for (const s of schema.split(';').map(s => s.trim()).filter(s => s.length > 0 && !s.startsWith('--'))) {
    try { await conn.query(s); } catch { /* ignore expected */ }
  }
  await conn.query('SET FOREIGN_KEY_CHECKS = 1');

  // Load procs
  const procContent = fs.readFileSync(path.join(process.cwd(), 'sqlscripting/storedprocedures.sql'), 'utf-8')
    .replace(/DELIMITER\s+\$\$/gi, '').replace(/DELIMITER\s+;/gi, '');
  for (const s of procContent.split('$$').map(s => s.trim()).filter(s => s.length >= 10)) {
    try { await conn.query(s.replace(/definer\s*=\s*`?[^`\s]+`?@`?[^`\s]+`?\s*/gi, '')); } catch { /* ignore */ }
  }
  ...
  await conn.query(`INSERT INTO quadrats (PlotID,QuadratName,StartX,StartY,DimensionX,DimensionY,Area,QuadratShape)
    VALUES (?,'Q01',0,0,10,10,100,'square')`, [plotID]);
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    await conn.query(`INSERT INTO coremeasurements (StemGUID,CensusID,MeasuredDBH,MeasuredHOM,MeasurementDate,IsValidated,IsActive)
      VALUES (?,?,?,1.3,'2024-06-15',1,1)`, [sguid, c1, 10 + i]);
  }
  ...
  const [cmRows] = await conn.query<mysql.RowDataPacket[]>(
    'SELECT CoreMeasurementID, RawTreeTag, RawCodes FROM coremeasurements WHERE CensusID=? AND StemGUID IS NOT NULL ORDER BY RawTreeTag', [c2]);
  const [attrRows] = await conn.query<mysql.RowDataPacket[]>(
    `SELECT ca.CoreMeasurementID, ca.Code FROM cmattributes ca
     ...
     WHERE cm.CensusID=? ORDER BY ca.CoreMeasurementID, ca.Code`, [c2]);
  ...
  const getCodes = (tag: string) => {
    ...
    if (!cm) return [];
    return attrRows.filter((r: any) => r.CoreMeasurementID === cm.CoreMeasurementID).map((r: any) => r.Code).sort();
  };
