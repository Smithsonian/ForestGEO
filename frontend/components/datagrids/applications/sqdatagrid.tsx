...
export default function SubquadratsDataGrid() {
  ...
  const addNewRowToGrid = () => {
    const id = randomId();
    // New row object
    const nextSubQuadratID = (rows.length > 0 ? rows.reduce((max, row) => Math.max(row.subquadratID, max), 0) : 0) + 1;
    const nextSQIndex = (rows.length > 0 ? rows.reduce((max, row) => Math.max(row.sqIndex, max), 0) : 0) + 1;
