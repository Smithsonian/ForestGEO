...
const SelectFormType: React.FC<SelectFormTypeProps> = ({ externalState, updateExternalState, updateExternalHeaders }) => {
  const colors: Record<string, 'neutral' | 'primary'> = {
    ...
    // Dead key: FormGroups exposes only 'Database Forms', so the legacy ctfsweb
    // CSV download this coloured no longer exists. Publishing to the Smithsonian
    // database is now "Publish census" on the census overview page.
