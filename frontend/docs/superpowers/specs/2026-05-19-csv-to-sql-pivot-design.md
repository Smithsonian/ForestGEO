...
The app's schema (`frontend/sqlscripting/tablestructures.sql:666-748`) already carries most of what v2 re-derives: validated measurements in `coremeasurements` with resolved `MeasuredDBH`/`MeasuredHOM`, identity in `trees`/`stems`, and materialized codes in `cmattributes`. The important exception is CTFS `DBH.PrimaryStem`: the app does **not** store a separate `PrimaryStem` field today. It stores uploaded codes in `coremeasurements.RawCodes` and valid materialized codes in `cmattributes`; the export must derive CTFS `PrimaryStem` from the app attribute catalog and must not treat the `main` / `secondary` marker codes as ordinary `DBHAttributes`.
...
Operator workflow: in the app, choose a finished census → click "Export to CTFS" → download `.sql` → run against the on-prem MySQL.
...
|---|---:|---|
| `destinationPlotID` | yes | CTFS destination `Census.PlotID`. Do **not** default this to the app `plotID`; the values are not guaranteed to match. Future work can replace this with a stored per-site mapping. |
...
6. A measurement has both a `main` marker and a `secondary` marker after joining `cmattributes.Code` to app `attributes.Description`.
7. A row needs CTFS `SubSpeciesID` but the export cannot map it. In the MVP, reject rows where the app species row has non-null `SubspeciesName` or `SubspeciesAuthority`; do not silently export `SubSpeciesID = NULL`.
8. Any destination-bound value exceeds the legacy CTFS column width: `Tree.Tag` 10 chars, `Stem.StemTag` 32 chars, `Quadrat.QuadratName` 8 chars, `TSMAttributes.TSMCode` 10 chars, `DBH.Comments` 128 chars. Fail in the app before generating SQL so the researcher gets row-level feedback.
9. Zero exportable rows remain after the above filters.
...
7. **Stage 1 — two temporary staging tables**:
   ...
   - `staging_attributes` — one row per **non-marker** `cmattributes` entry, referencing `staging_measurements` by app-side `CoreMeasurementID`.
...
- Natural keys for destination lookup: `Tag`, `StemTag`, `Mnemonic`, `QuadratName`, `PlotCensusNumber`.
- Resolved fields from app: `DBH` (already null if zero, already validated), `HOM` (already chosen — no inheritance needed), `ExactDate`, `Comments`, `LX`, `LY`, `PrimaryStem` (one of `'main'`/`'secondary'`/`NULL`, derived during export from `cmattributes` joined to app `attributes.Description`).
- Destination-resolved holes (populated by Stage 2): `TreeID`, `StemID`, `SpeciesID`, `QuadratID`, `CensusID`, `DBHID`.
...
**PrimaryStem and marker-code split:** app ingestion currently materializes every valid uploaded code into `cmattributes`; it does not remove `main` / `secondary` markers and does not store a separate app-side `PrimaryStem`. The export endpoint must therefore split codes:

- Codes whose app `attributes.Description` lowercases to `main` or `secondary` set `staging_measurements.PrimaryStem`.
- Those marker codes are **not** inserted into `staging_attributes`, so they do not become CTFS `DBHAttributes`.
- All other codes become `staging_attributes` rows.
- A measurement carrying both marker types is an app-side 400 response before SQL generation.

This preserves legacy CTFSWeb upload behavior: marker codes populate `DBH.PrimaryStem`; ordinary codes populate `DBHAttributes`.
...
Stage 0 also probes the destination `DBHAttributes` shape:

```sql
SELECT COUNT(*)
  INTO @dbhattrs_has_census_id
...
Real CTFSWeb databases may be either pre- or post-`DBCHANGES2014f`; the generated artifact must support both.
...
2. **Tree-key uniqueness on the destination plot**: `tree_lookup.TreeCount > 1` for any referenced `(Tag, SpeciesID, SubSpeciesID)`. CTFS DDL has no plot-scoped Tag uniqueness; legacy data may have duplicates.
3. **Stem uniqueness on the destination tree**: `stem_lookup.StemCount > 1` for any referenced `(TreeID, StemTag)`.
4. **Unknown mnemonic** on CTFS Species.
...
9. **No ambiguous pre-existing Tree for new tree keys**: before Stage 6 inserts a new CTFS `Tree`, fail if a row already exists in `Tree` with the same `(Tag, SpeciesID, SubSpeciesID)` but is not uniquely resolved by the plot-scoped `tree_lookup`. Otherwise Stage 6's join-back could attach staging rows to the wrong pre-existing tree.
10. **Destination string lengths** as a second line of defense: enforce CTFS widths for `Tag`, `StemTag`, `QuadratName`, `TSMCode`, and `Comments` even though the app endpoint should have rejected these earlier.
11. **DBHAttributes schema support**: `@dbhattrs_has_census_id` must be either 0 or 1. If `information_schema` returns anything unexpected, SIGNAL before writes.
...
INSERT INTO Tree (Tag, SpeciesID, SubSpeciesID)
  SELECT nt.Tag, nt.SpeciesID, NULL
    FROM (
      SELECT Tag, SpeciesID, MIN(TempID) AS first_temp_id
        FROM staging_measurements
       WHERE TreeID IS NULL
       GROUP BY Tag, SpeciesID
    ) nt
...
UPDATE staging_measurements t
  JOIN Tree tr ON tr.Tag = t.Tag
              ...
              AND tr.SubSpeciesID IS NULL
  SET t.TreeID = tr.TreeID
  WHERE t.TreeID IS NULL;
```

`SubSpeciesID` defaults to NULL only after the app endpoint has rejected rows that need a real subspecies mapping. Silent loss of subspecies is not allowed.
...
IF @dbhattrs_has_census_id = 1 THEN
  INSERT INTO DBHAttributes (CensusID, TSMID, DBHID)
    SELECT @target_census_id, TSMID, DBHID
...
`DBHAttributes.CensusID` is schema-dependent. The canonical fixture declares it, but `DBCHANGES2014f.sql` removes it and the legacy upload script inserts only `(DBHID, TSMID)`. The artifact must use the Stage 0 capability probe and choose the correct insert shape for the destination database.
...
Suzanne's requested workflow explicitly includes "Create ViewFullTable" after upload. The artifact cannot silently stop at `Tree` / `Stem` / `DBH` / `DBHAttributes` and leave CTFSWeb's reporting table stale.

Implementation requirement: include a final ViewFullTable handling step once the target-site convention is confirmed. Acceptable outcomes are:

- Call the existing CTFSWeb ViewFullTable rebuild routine if every destination site has one with a stable name/signature.
- Emit the SQL needed to refresh the target census's ViewFullTable rows if there is no stable routine.
- If ViewFullTable must be rebuilt manually by a site administrator, emit a final SELECT warning that says the data tables were loaded but ViewFullTable still needs to be rebuilt before researchers use CTFSWeb exports.

This is a release gate for production use, not a cosmetic follow-up.
...
3. `DELETE` `DBHAttributes` for the target census by joining through `DBH` (`DELETE da FROM DBHAttributes da JOIN DBH d ON d.DBHID = da.DBHID WHERE d.CensusID = @target_census_id;`). This works whether or not `DBHAttributes` has its own `CensusID` column.
...
- Returns 400 when a measurement has both `main` and `secondary` marker attributes.
- Returns 400 when a row needs subspecies mapping that the MVP cannot emit.
...
- PrimaryStem split: marker code goes to `staging_measurements.PrimaryStem`; non-marker code goes to `staging_attributes`; marker code does not appear in `staging_attributes`.
...
- DBHAttributes insert works against both schema variants: with `CensusID` present and with `CensusID` removed per `DBCHANGES2014f.sql`.
...
- ViewFullTable handling test is added once Suzanne confirms the target refresh convention.

### Performance

Generate a synthetic 44k-row app-database fixture matching SERC scale. Measure end-to-end wall-clock from endpoint hit to destination COMMIT. Record as **baseline**, not budget. Budget set in a follow-up after the measurement lands.
...
- Full `SubSpeciesID` mapping/emission from the app. MVP rejects rows that require subspecies instead of silently dropping it.
- Deletion of `lib/csv-to-sql.ts` (v1) and its re-export shims.
- Deletion of the `CTFSWebForms` legacy CSV form-type.

## Open Questions

- **Destination CTFS PlotID source:** MVP accepts `destinationPlotID` as an explicit parameter. A production UI should probably store this mapping once per app plot/site instead of making the operator type it every export.
- **ViewFullTable rebuild convention:** Suzanne's note says CTFSWeb upload created ViewFullTable, but the exact routine/table-refresh convention is not documented in this spec yet.
- **PrimaryStem marker convention:** The export assumes attributes whose description is exactly `main` or `secondary` are marker codes. Confirm whether every site follows that convention and whether comparisons should be case-insensitive only or also tolerate punctuation/extra words.
...
These are intentionally plain-language questions to confirm the operational target before implementation:

1. When someone downloads the SQL from the ForestGEO app, should they choose the CTFSWeb plot id themselves, or do you expect the app to already know which CTFSWeb plot id matches the app plot?
2. Are the CTFSWeb databases all using the newer schema where `DBHAttributes` has only `DBHID` and `TSMID`, or do some still have a `CensusID` column in `DBHAttributes`?
3. In CTFSWeb, are `main` and `secondary` always stored as TSM attribute descriptions? Should those codes only set `DBH.PrimaryStem`, and not be inserted into `DBHAttributes`?
4. After the script adds the census records to `Tree`, `Stem`, `DBH`, and `DBHAttributes`, how should CTFSWeb make those new records visible in `ViewFullTable`? For example, should the script call an existing stored procedure, run a separate SQL file, or tell the site administrator to refresh `ViewFullTable` manually?
5. Do any sites actively use subspecies on Tree rows? If yes, where should the ForestGEO app get the CTFSWeb `SubSpeciesID` from?
6. Can a single CTFSWeb MySQL database contain multiple plots with overlapping tree tags? This affects how carefully the script must scope Tree lookup and new Tree insertion.
7. Who should be allowed to generate a normal export, and who should be allowed to generate a reload export that deletes and replaces an existing census in CTFSWeb?
8. Should reloads ever delete orphaned `Stem` or `Tree` rows, or should the script only replace `DBH` and `DBHAttributes` and leave identity cleanup to a separate manual step?
9. What is the largest finished census we should expect researchers to export, roughly: 10k rows, 50k rows, 100k rows, or larger?
