...
| lx       | Strongly recommended | X coordinate within the quadrat | 5.2 |
| ly       | Strongly recommended | Y coordinate within the quadrat | 8.7 |
...
:::caution
**Give every row a stem tag.** The upload screen's header guide marks `stemtag` optional, but a
row whose stem tag is blank is rejected during ingestion. Most sites use `0` for a
single-stemmed tree and `1`, `2`, … for the rest.

**Supply the coordinates too.** A CSV row without `lx` and `ly` will ingest, but the stem cannot
be placed on the plot and will be flagged by the plot-boundary and coordinate-drift checks. They
are only *rejected* outright on ArcGIS workbook imports.
:::
...
- Required on every row: `tag`, `stemtag`, `spcode`, `quadrat`, `date`
- Also supply `lx` and `ly` — not enforced on CSV, but the stem cannot be placed without them
