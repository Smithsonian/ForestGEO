...
- Personnel are recorded per census; species, stem codes and quadrats belong to the plot and carry across every census
...
An intended shortcut for carrying data from one census into the next.

**Key points:**

- **Not currently functioning** — the census creation form collects only a description and dates, and offers no rollover options
- Most supporting data needs no rollover anyway: species, stem codes and quadrats belong to the plot
- Personnel, which are census-scoped, must be re-added by hand under Stem & Plot Details → Personnel
...
- Kept in `coremeasurements` alongside successful rows, distinguished by having no resolved stem; the original values from your file are preserved
- Common causes: invalid species codes, missing fields, duplicate records
- Can be reviewed, edited, and re-ingested
- Reviewed under Census Hub → View Errors
...
- Census-dependent: recorded separately for each census
...
- Belong to the plot, not to a census — defined once and used by every census in that plot
...
- Reached from Census Hub → View Errors
...
- Runs in the background after a measurements upload, not as part of it
