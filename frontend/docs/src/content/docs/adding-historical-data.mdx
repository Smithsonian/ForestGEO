...
Two columns behave differently from what the upload screen implies. **`stemtag` is required** — a
row with a blank stem tag is rejected during ingestion, even though the header guide marks it
optional. Most sites use `0` for a single-stemmed tree. Conversely `lx` and `ly` are only
*enforced* on ArcGIS workbook imports; a CSV without them will ingest, but the stem cannot be
placed on the plot and will be flagged by the boundary and coordinate-drift checks. Supply them.
:::
...
| `lx`       | Strongly recommended | Decimal | X-coordinate within quadrat — see the note below |
| `ly`       | Strongly recommended | Decimal | Y-coordinate within quadrat — see the note below |
