...
When you first start using the application, you'll need to add historical data from previous censuses. 
This guide explains the data types, file formats, and upload process.
...
:::note
You must complete the Fixed Data uploads before you can upload measurements. 
The measurements form references Fixed Data records.
:::
...
There are **three** core data types that are required before you can upload measurements for a census. 
The app is designed to accept file uploads (called `forms`) in either CSV or TSV format.

> **Fixed Data is not census-dependent!**

They are:
- **Stem Codes**: A list of shorthand codes designating attributes that can be assigned to a tree/stem object
- **Personnel**: A list of personnel working on the census. Optionally, you can also add/designate roles to each user to better denote job distribution across the census.
- **Quadrats**: A breakdown of the plot into smaller area segments
- **Species**: A list of species found in the plot. Each species is assigned a shorthand **Species Code** to allow quick assignment when performing collection
...
| **Personnel**               | Field staff for the census       | No                 |
