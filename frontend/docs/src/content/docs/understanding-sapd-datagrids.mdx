...
Edits to measurement rows — whether you start in View Data, View Errors, or the Failed Measurements view — all flow through one shared pipeline. The pieces you'll see:

- **Impact Summary dialog.** Before any write, a dialog shows exactly which fields will change, how many other rows are affected (some edits cascade), and any validation warnings the change would raise. The preview is **tamper-proof** — what you see is what gets applied.
- **Typed confirmation for destructive edits.** Deletes and mass changes ask you to type `APPLY N` (where N is the row count) as a guard against accidental clicks.
- **Per-row revert.** After Apply, the row menu offers a Revert action that rolls back the last change on that row.
...
:::note
Most Fixed Data grids (Stem Codes, Personnel, Quadrats, Species List) still use the simpler immediate-save flow — there's no Impact Summary because each row stands alone. The unified pipeline applies to **measurement** edits, where one change can ripple through validations and other rows.
:::
