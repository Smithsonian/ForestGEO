...
| ----------------------------------------------------------------- | ------------------------------------------------------------------------ | ------------- |
| **ValidateDBHGrowthExceedsMax**                                   | Flag DBH measurements showing growth exceeding 65 mm/year from the previous census | Enabled       |
| **ValidateDBHShrinkageExceedsMax**                                | Flag DBH measurements showing shrinkage of at least 5% per year from the previous census | Enabled       |
...
These flag growth or shrinkage for review using the measurement dates and the immediately preceding census number. The prior measurement must be validated successfully.

- **Growth > 65 mm/year** *(V1)*: DBH increase converted to millimetres, divided by elapsed years. Exactly 65 mm/year does not flag.
- **Shrinkage of at least 5% per year** *(V2)*: DBH change divided by prior DBH and elapsed years. An annual relative change of exactly −0.05 flags.
...
Elapsed years equal the difference between the two measurement dates in days divided by 365.25.
Rates are only annualised when the measurements are at least 365 days apart. Measurements less
than a year apart, or with a missing date, are judged on total change instead: growth above
65 mm, or a drop of more than 5% of the prior DBH. Annualising a few weeks of change would turn
ordinary measurement precision into an implausible yearly rate.

Both DBHs must be at least 10 mm (1 cm). Different non-missing HOM values suppress the check;
either HOM may be missing. Missing DBH, reversed dates, intervals longer than 20 years (almost
always a mistyped year), and exempt statuses on either measurement skip the comparison.
A skipped comparison is not evidence that the measurement passed a growth check.

DBH reruns retain occurrence rows, resolve stale findings, and reopen findings that still violate
the rule. The original creation time and latest prior comparison are retained; this is not an
immutable history. Previously overridden DBH findings can be flagged again.
