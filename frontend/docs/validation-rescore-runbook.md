...
Mason requires all four simplicity fixes before merge (September 18). The release record must name the tested revision before apply:

- The override modal now calls one admin-only, site-authorized `/api/validations/override` operation. The server acquires the shared census lock, checks active work, and commits error resolution, override markers, validity, and both materialized views in one transaction. View updates target only overridden measurements in bounded batches within that transaction; the UI then reloads results without rebuilding the census again. A real-MySQL regression injects a failure at the final view refresh and verifies complete rollback and successful retry. This prevents new partial overrides; it does not repair or prove the intent of historical partially applied operations. Re-scoring must still review historical discrepancies and valid-to-invalid holds.
- New attempts use `validation_runs.RescoreAttemptID` for commit evidence. `Notices` stores non-failing user messages; `ErrorMessages` stores failures. The browser runner, upload worker, run-status API and badge keep notices separate, including on failed runs. Migration `2026-09-18-01-separate-validation-run-messages` adds both nullable columns before deployment; canonical provisioning and the schema contract require them. Keep these columns and historical records after tool retirement. Reconciliation still reads old `dbh-rescore-attempt:` evidence, and the API filters it from legacy display messages; older completed records remain readable without rewriting history.
- `dbh-rescore-preflight.ts` owns upload, running-validation, eligible-pending and background-job checks for both advisory reads and execution. Execution repeats the same checks for current and immediate-prior censuses after acquiring their locks. Retire this dedicated helper with the one-time tool.
- Test schema loading uses the existing provisioning `splitSqlFile` parser, including quoted semicolons, followed by an independent canonical-table completeness check. The schema-loader branch's compensating table definitions are removed; tests use canonical definitions.

Record verification evidence and reviewer in the completion record before production apply. The protected 25-scope September 17 rehearsal and rollback comparison passed at the archived revision `32516ba3`. These simplicity fixes change executable code and message storage; that archived packet is immutable historical evidence. Record new migration, recovery, preflight, notice and schema-loader checks for the final revision and repeat the copy rehearsal before production apply. Do not represent the older packet as proof of unchanged runtime.
...
The previous local snapshot was unavailable; a newly approved protected production copy was exported and restored on September 17. Its 25-scope legacy baseline, fixed-prior comparison, annualized sweep, timeout recovery, and largest-census injected rollback have completed. The full legacy restoration comparison also passed, with all 25 scopes returned to their legacy baseline and no comparison issues. The tracked [September 17 release record](../../docs/notes/2026-09-17-dbh-release-status.md) distinguishes finished checks from remaining production work.
...
| `frontend/lib/validations/dbh-rescore.ts`, `dbh-rescore-cli.ts`, `dbh-rescore-sweep.ts`, `dbh-rescore-preflight.ts`                                | Dedicated execution, CLI, ordering, reconciliation, and artifact orchestration                                 |
