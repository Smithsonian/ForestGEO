...
async function listEnabledValidations(connectionManager: ConnectionManager, schema: string): Promise<EnabledValidation[]> {
  const query = safeFormatQuery(schema, 'SELECT ValidationID, ProcedureName, Description, Definition FROM ??.sitespecificvalidations WHERE IsEnabled = 1');
...
export async function runCensusValidations(connectionManager: ConnectionManager, params: RunCensusValidationsParams): Promise<CensusValidationSummary> {
  ...
  if (failedSteps === 0) {
    try {
      ...
      failedSteps++;
      errors.push(`Failed to update validated rows: ${error.message}`);
  ...
  try {
    ...
    failedSteps++;
    errors.push(`Failed to refresh summary views: ${error.message}`);
    ailogger.error(`[ValidationOrchestrator] Failed to refresh summary views: ${error.message}`);
  }
