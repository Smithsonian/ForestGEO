...
export type DBHValidationSkipCounts = { skippedNoInterval: number; skippedMissingDate: number; skippedZeroInterval: number; skippedNegativeInterval: number };
...
const empty: DBHValidationSkipCounts = { skippedNoInterval: 0, skippedMissingDate: 0, skippedZeroInterval: 0, skippedNegativeInterval: 0 };
...
export function parseDbhValidationSkipCounts(result: unknown): DBHValidationSkipCounts {
  const find = (v: any): any =>
    Array.isArray(v) ? v.map(find).find(Boolean) : v && typeof v === 'object' && ('SkippedNoInterval' in v || 'SkippedMissingDate' in v) ? v : null;
  const row = find(result);
  if (!row) return { ...empty };
  const n = (key: string) => (Number.isFinite(Number(row[key])) ? Number(row[key]) : 0);
  return {
...
async function scrub(tx: TxExecutor, schema: string, id: number, params: ValidationExecutionParams, guard: boolean) {
...
export async function prepareDBHValidationRunInTransaction(input: {
  ...
  const { schema, tx, validationID, params, requireActiveStemGUID = false } = input;
  ...
  await scrub(tx, schema, validationID, params, requireActiveStemGUID);
}
export async function runSharedDBHChangeValidationsInTransaction(input: {
  ...
  if (growth) await scrub(tx, schema, DBH_CHANGE_VALIDATION_IDS.growth, params, requireActiveStemGUID);
  if (shrinkage) await scrub(tx, schema, DBH_CHANGE_VALIDATION_IDS.shrinkage, params, requireActiveStemGUID);
  const result =
    growth || shrinkage
