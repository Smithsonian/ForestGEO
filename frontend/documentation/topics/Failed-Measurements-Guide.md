...
### Missing Measurement Failures

| Failure Reason            | Fields     | Cause                               | How to Fix              |
| ------------------------- | ---------- | ----------------------------------- | ----------------------- |
| **Missing Codes and DBH** | Codes, DBH | No attribute codes AND no DBH value | Add either codes or DBH |
| **Missing Codes and HOM** | Codes, HOM | No attribute codes AND no HOM value | Add either codes or HOM |
...
- **Failed Measurements**: Number of records that failed validation
- **Temporary Measurements**: Records waiting to be processed (from a previous attempt)
...
### Scenario 3: "Missing Tag" or "Missing StemTag"

**Cause:** Empty tag fields in your original file.

**Solution:**

1. Edit each failed record individually
2. Add the correct tag values
3. Save and reingest

Or:

1. Clear failed measurements
2. Fix your original file
3. Re-upload

---

### Scenario 4: Multiple failure reasons on same record

**Cause:** A single record has several issues.

**Solution:**

1. Address ALL failure reasons before reingestion
2. For example, if a record shows:
   - "SpCode invalid"
   - "Missing X"
   - "Missing Y"
3. You need to:
   - Add the species to Species List
   - Provide X and Y coordinates
4. Only then reingest

---

### Scenario 5: Same failures keep appearing after reingest
...
## Failed Measurements Data Grid

The Failed Measurements view shows all failure information:

| Column              | Description                             |
| ------------------- | --------------------------------------- |
| **ID**              | Unique identifier for the failed record |
| **Tree Tag**        | The tree tag value                      |
| **Stem Tag**        | The stem tag value                      |
| **Species Code**    | The species code (may be invalid)       |
| **Quadrat**         | The quadrat name (may be invalid)       |
| **X, Y**            | Coordinates (may be missing)            |
| **DBH**             | Diameter measurement                    |
| **HOM**             | Height of measurement                   |
| **Date**            | Measurement date                        |
| **Codes**           | Attribute codes                         |
| **Failure Reasons** | List of why record failed               |

### Filtering Failed Measurements

You can filter the failed measurements grid to find specific issues:

- Filter by failure reason
- Filter by species code
- Filter by quadrat
- Filter by tag numbers

---

## Preventing Failed Measurements

### Before Uploading:

1. **Verify all reference data exists**
   - Check that all species codes are in Species List
   - Check that all quadrat names are defined
   - Check that all attribute codes exist

2. **Validate your file**
   - No empty required fields
   - Correct date format (YYYY-MM-DD)
   - Valid numeric values for coordinates and measurements

3. **Check for duplicates**
   - Each TreeTag + StemTag should be unique per census

### Download Error File

After upload, you can download a file containing only the failed records with their failure reasons. This helps you:

- Review errors offline
- Fix issues in a spreadsheet
...
### Q: Why do some records fail even though the species/quadrat exists?

**A:** Check for:

- Case sensitivity (codes must match exactly)
- Leading/trailing spaces
- The data being added to the wrong plot or census

---

## Troubleshooting

### Reingestion keeps failing with same errors

1. Verify the fix was actually saved (refresh the page)
2. Check for case sensitivity issues
3. Ensure you're in the correct plot/census context
4. Check the database directly (admin only)

### Cannot find the failed measurements

1. Make sure correct plot and census are selected
2. Check if they were already cleared
3. Look in the Measurements Hub view

### "Clear Failed" button doesn't work
