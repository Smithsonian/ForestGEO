...
async function validateFindAllInvalidSpeciesCodes(plotID: number | null, censusID: number | null) {
  const conn = await getConn();
  try {
    await conn.beginTransaction();
    const query = 'CALL ValidateFindAllInvalidSpeciesCodes(?, ?)';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const validationResponse = await validateFindAllInvalidSpeciesCodes(plotID, censusID);
