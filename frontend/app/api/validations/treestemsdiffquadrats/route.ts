...
async function validateFindTreeStemsDiffQuadrats(plotID: number | null, censusID: number | null) {
  const conn = await getConn();
  try {
    await conn.beginTransaction();
    const query = 'CALL ValidateFindTreeStemsInDifferentQuadrats(?, ?)';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const validationResponse = await validateFindTreeStemsDiffQuadrats(plotID, censusID);
