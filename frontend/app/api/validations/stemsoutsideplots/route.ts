...
async function validateFindStemsOutsidePlots(plotID: number | null, censusID: number | null) {
  const conn = await getConn();
  try {
    await conn.beginTransaction();
    const query = 'CALL ValidateFindStemsOutsidePlots(?, ?)';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const validationResponse = await validateFindStemsOutsidePlots(plotID, censusID);
