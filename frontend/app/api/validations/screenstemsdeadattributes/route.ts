...
async function validateScreenStems(plotID: number | null, censusID: number | null) {
  const conn = await getConn();
  try {
    await conn.beginTransaction();
    const query = 'CALL ValidateScreenStemsWithMeasurementsButDeadAttributes(?, ?)';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const validationResponse = await validateScreenStems(plotID, censusID);
