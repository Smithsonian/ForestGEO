...
async function ValidateFindStemsInTreeWithDifferentSpecies(plotID: number | null, censusID: number | null) {
  const conn = await getConn();
  try {
    await conn.beginTransaction();
    const query = 'CALL ValidateFindStemsInTreeWithDifferentSpecies(?, ?)';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const validationResponse = await ValidateFindStemsInTreeWithDifferentSpecies(plotID, censusID);
