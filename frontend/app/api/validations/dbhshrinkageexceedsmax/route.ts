...
async function validateDBHShrinkageExceedsMax(plotID: number | null, censusID: number | null) {
  const conn = await getConn();
  try {
    await conn.beginTransaction();
    const query = 'CALL ValidateDBHShrinkageExceedsMax(?, ?)';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const validationResponse = await validateDBHShrinkageExceedsMax(plotID, censusID);
