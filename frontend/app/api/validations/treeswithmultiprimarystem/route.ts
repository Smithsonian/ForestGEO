...
async function validateFindTreesMultiPrimaryStems(plotID: number | null, censusID: number | null) {
  const conn = await getConn();
  try {
    await conn.beginTransaction();
    const query = 'CALL ValidateFindTreesWithMultiplePrimaryStems(?, ?)';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const validationResponse = await validateFindTreesMultiPrimaryStems(plotID, censusID);
