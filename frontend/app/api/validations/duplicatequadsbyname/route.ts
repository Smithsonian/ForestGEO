...
async function validateFindDuplicateQuadratsByName(plotID: number | null, censusID: number | null) {
  const conn = await getConn();
  try {
    await conn.beginTransaction();
    const query = 'CALL ValidateFindDuplicatedQuadratsByName(?, ?)';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const validationResponse = await validateFindDuplicateQuadratsByName(plotID, censusID);
