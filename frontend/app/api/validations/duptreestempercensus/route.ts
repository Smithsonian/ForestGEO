...
async function validateDuplicateTreeStemByCensus(plotID: number | null, censusID: number | null) {
  const conn = await getConn();
  try {
    await conn.beginTransaction();
    const query = 'CALL ValidateFindDuplicateStemTreeTagCombinationsPerCensus(?, ?)';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const validationResponse = await validateDuplicateTreeStemByCensus(plotID, censusID);
