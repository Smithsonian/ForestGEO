...
async function validateDBHGrowthExceedsMax(plotID: number | null, censusID: number | null) {
  const conn = await getConn();
  try {
    await conn.beginTransaction();
    const query = 'CALL ValidateDBHGrowthExceedsMax(?, ?)';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const validationResponse = await validateDBHGrowthExceedsMax(plotID, censusID);
