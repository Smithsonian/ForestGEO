...
import { attributesFields } from '@/config/sqlrdsdefinitions/tables/attributerds';
...
export async function GET(request: NextRequest): Promise<NextResponse<{
  ...
  try {
    ...
    const mapper = MapperFactory.getMapper<AttributesResult, AttributesRDS>('Attributes');
    const attributeRows = mapper.mapData(paginatedResults);
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    const changedFields = attributesFields.filter(field => computeMutation('attributes', oldRow, newRow));
    ...
    const updatesNeeded = changedFields.flatMap(field => {
      ...
      case 'description':
      case 'status':
        return mysql.format('UPDATE ?? SET ?? = ? WHERE Code = ?', [`${schema}.attributes`, field, newRow[field], newRow.code]);
      default:
        return null;
      } 
