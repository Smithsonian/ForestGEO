...
import {PersonnelGridColumns, PersonnelRDS, StyledDataGrid} from "@/config/sqlmacros";
import {EditToolbarProps} from "@/config/macros";
...
function EditToolbar(props: Readonly<EditToolbarProps>) {
  ...
  const handleRefresh = async () => {
    try {
      const response = await fetch(`/api/fixeddata/personnel?page=${paginationModel.page}&pageSize=${paginationModel.pageSize}`, {
        method: 'GET'
      });
      if (!response.ok) {
...
export default function PersonnelPage() {
  ...
  const fetchPaginatedData = async (pageToFetch: number) => {
    setRefresh(true);
    try {
      const response = await fetch(`/api/fixeddata/personnel?page=${pageToFetch}&pageSize=${paginationModel.pageSize}`);
      const data = await response.json();
      if (!response.ok) {
  ...
  const processRowUpdate = React.useCallback(
    async (newRow: GridRowModel, oldRow: GridRowModel): Promise<GridRowModel> => {
      ...
      if (newRow.personnelID === '') {
        throw new Error("Primary key 'PersonnelID' cannot be empty!");
      }

      try {
        let response, responseJSON;
        // If oldRow code is empty, it's a new row insertion
        if (oldRow.personnelID === '') {
          response = await fetch('/api/fixeddata/personnel', {
            method: 'POST',
          ...
          if (mutation) {
            response = await fetch(`/api/fixeddata/personnel?personnelID=${oldRow.personnelID}`, {
              method: 'PATCH',
              headers: {'Content-Type': 'application/json'},
              body: JSON.stringify(newRow),
            });
