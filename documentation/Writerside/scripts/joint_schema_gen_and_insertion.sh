...
if [ "$#" -ne 3 ]; then
    echo "Usage: $0 <source_sql_file> <destination_schema_name> <mysql_config_file>"
    exit 1
fi
...
while (/DROP TABLE IF EXISTS `([^`]+)`;\s*\/\*[^*]*\*\/;\s*\/\*[^*]*\*\/;\s*CREATE TABLE `([^`]+)` \((.*?)\)\s*ENGINE=[^;]+;/sg) {
    ...
    print "Captured table: $table_name\n";
    while ($table_def =~ /FOREIGN KEY \(`[^`]+`\) REFERENCES `([^`]+)`/sg) {
        my $referenced_table = $1;
        push @{$foreign_keys{$table_name}}, $referenced_table;
        print "Foreign key reference from $table_name to $referenced_table\n";
    }
}
...
while (/DROP TABLE IF EXISTS `([^`]+)`;/sg) {
    my $table_name = $1;
    my $drop_def = $&;
    $drop_statements{$table_name} = $drop_def;
    print "Captured DROP TABLE: $table_name\n";
}
...
sub visit {
    ...
    $visited{$table} = 1;
    if (exists $foreign_keys{$table}) {
        visit($_) for @{$foreign_keys{$table}};
    }
    push @sorted_tables_with_fk, $table;
    print "Visited table: $table\n";
}
...
while (/INSERT INTO `([^`]+)` VALUES \((.*?)\);/sg) {
    ...
    my @rows = $values =~ /\((.*?)\)/sg;
    for my $row (@rows) {
        push @{$insert_statements{$table}}, "($row)";
    }
    print "Captured INSERT INTO for table: $table\n";
}

open my $insert_fh, ">", "'$extracted_inserts_file'" or die "Could not open file: $!";
foreach my $table (keys %insert_statements) {
    foreach my $row (@{$insert_statements{$table}}) {
        print $insert_fh "INSERT INTO `$table` VALUES $row;\n";
    }
}
close $insert_fh;
...
sub visit_drop {
    ...
    $visited{$table} = 1;
    if (exists $foreign_keys{$table}) {
        visit_drop($_) for @{$foreign_keys{$table}};
    }
    push @sorted_drop_tables, $table;
    print "Visited drop table: $table\n";
}
...
verify_row_count() {
    ...
    if [ "$actual_count" -eq "$expected_count" ]; then
        echo "Row count verification successful for table $table: $actual_count rows."
    else
