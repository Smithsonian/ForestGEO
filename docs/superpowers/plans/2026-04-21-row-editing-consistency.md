...
7. **Apply primitive boundary** — `applyEdit` is the endpoint-facing single-row helper and always owns lock + transaction. `applyEditInTransaction` is the shared writer primitive for revision apply and compatibility shims when they already own a transaction. Revert owns lock/transaction separately and calls `applyEditInTransaction` with `operationType: 'revert'` after reconstructing the pre-edit user payload from the ledger. No batch path may call `applyEdit` in a loop.
8. **Revert scope** — Phase 1 single-row undo is allowed only for ledger entries flagged `revertable=true`. Revert rebuilds the user-facing payload from `beforeState` raw columns (`RawTreeTag`, `RawSpCode`, `RawStemTag`, `RawQuadrat`, `MeasurementDate`, `MeasuredDBH`, `MeasuredHOM`, `RawCodes`, `Description`, and `stems.LocalX`/`LocalY` where present) and replays it through the normal analyzer → writer path. It does **not** perform row-level restore by primary key: the analyzer re-resolves species/tree/stem/quadrat against live rows, so secondary surrogate keys such as `cmattributes.CMAID` and any `TreeID`/`StemGUID` created by the original edit are not preserved byte-for-byte. Rows the original edit created that are no longer referenced post-restore are cleaned up via `removeCreatedAfterStateSideEffects`. If the world has moved on enough that the restore plan fires cross-row or destructive effects, the endpoint returns `RevertDriftError` and the client re-confirms. Bulk revision ledger rows are marked `revertable=false` in the ledger and the endpoint rejects them up front.
...
| `frontend/config/editplan/scopeguard.ts` | Shared endpoint guards for schema/plot/census authorization and non-locking upload-session/validation-run conflict probes. Target-in-scope (ownership) is enforced implicitly by `analyzer.loadCurrentRow`, not by a separate `assertTargetInScope` export — its scoped WHERE clause plus `TargetNotFoundError` delivers the same 404 contract. |
...
| `frontend/config/editplan/revert.ts` | Looks up an `edit_operations` row, reconstructs the pre-edit user payload from the ledger's `beforeState` raw columns, replays it through `analyzeEdit` + `applyEditInTransaction` (`operationType: 'revert'`) inside one locked transaction, removes rows the original edit created that are no longer referenced, and links the original/revert ledger rows. If the restore plan has cross-row or destructive effects, throws `RevertDriftError` with the fresh plan for client re-confirmation. |
...
- [ ] `scopeguard.ts` exports `assertCanEditMeasurementScope` and `assertNoActiveMeasurementScopeConflict`. Target-in-scope (the `assertTargetInScope` check from the original plan) is enforced implicitly by the analyzer: `analyzer.loadCurrentRow` constrains its WHERE clause to `coremeasurements.CoreMeasurementID`, `coremeasurements.CensusID`, `census.PlotID`, and the `StemGUID IS [NOT] NULL` shape required by the dataType, translating a missed lookup to `TargetNotFoundError` (→ 404). The plan's `StemGUID IS NULL` (failed rows) / `StemGUID IS NOT NULL` (measurement rows) distinction is preserved via dataType routing in the analyzer.
- [ ] `/preview`: no-writes path — calls `assertCanEditMeasurementScope` and `assertNoActiveMeasurementScopeConflict` before `analyzeEdit`, then calls `analyzeEdit` with no transaction (the analyzer enforces target-in-scope internally). Returns `EditPlan`. 400 on bad body, 401 unauthenticated, 403 unauthorized scope, 404 target outside scope (via `TargetNotFoundError`), 422 on `DisallowedFieldError`, `InvalidClearError`, or `SpeciesNotFoundError`, 423 on active upload/validation conflict.
- [ ] `/apply`: body `{ schema, plotID, censusID, dataType, targetID, newRow, planHash }`. Rejects public `bypassPreview` flags. Calls `assertCanEditMeasurementScope` (the authoritative scope lock inside `applyEdit` supersedes a preview-style conflict probe), then `applyEdit` with `expectedPlanHash: body.planHash`. Translates exceptions: `HashDriftError` → 409 with `{ freshPlan }`; `ScopeLockHeldError` → 423; `DisallowedFieldError` / `InvalidClearError` / `SpeciesNotFoundError` → 422; `TargetNotFoundError` → 404.
...
- [ ] **Step 2: Implement `/apply/route.ts`** (same pattern, includes `planHash`, calls `assertCanEditMeasurementScope`; `applyEdit` performs the exclusive lock and translates `ScopeLockHeldError` to 423. Target-in-scope is enforced implicitly by the analyzer running inside `applyEditInTransaction`).
