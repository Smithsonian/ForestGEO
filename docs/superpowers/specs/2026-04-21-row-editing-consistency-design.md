...
type EditOperation = {
  ...
  revertable: boolean;
  dataType: EditPlanDataType;
  // Nullable: bulk-revision-row summary entries cover a whole batch and have
  // no single target measurement. Affected CoreMeasurementIDs live in
  // BeforeState JSON. Single-row and revert entries always set this.
...
Both affordances hit the same revert endpoint. Revert is itself a normal edit that goes through the shared apply helper: the endpoint reconstructs a user-facing payload from the ledger's `beforeState` raw columns (TreeTag, SpeciesCode, StemTag, QuadratName, DBH, HOM, date, coordinates, codes), feeds it back through `analyzeEdit` + the writer, and produces its own plan, ledger entry, and audit changelog. If the revert would itself cause a destructive effect (rare, but possible if the world has moved on — e.g. the original species or quadrat has since been deactivated), the endpoint returns `RevertDriftError` with the fresh plan; the client surfaces it and the user re-confirms.

Consequences of the replay-through-analyzer approach:

- Restored rows carry the user-visible field values that existed before the original edit (measured DBH, species assignment, tree/stem linkage, attributes), but secondary surrogate keys generated by the writer — `cmattributes.CMAID`, and the `StemGUID`/`TreeID` of any new rows the original edit created — are not preserved byte-for-byte. The analyzer resolves to whatever live row now matches the restored identity tuple, and the writer cleans up rows the original edit created that are no longer referenced (via `removeCreatedAfterStateSideEffects`).
- If the original edit's target row has been deactivated, moved out of scope, or had its referenced species/quadrat/tree deactivated, the revert endpoint returns an error rather than silently forcing a broken restore. Recovery for those cases goes through the Recent Changes Explorer.
- Because revert calls the same writer, the row-level validation, ingestion-error refresh, and materialized-view refresh run on the restored row.
